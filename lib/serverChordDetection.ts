import { barDurationSec } from '@/lib/metronomeAudio'
 ...
 *      only has a user-supplied BPM + time signature, bar length is derived
 *      the same way the client computes bar numbers (barDurationSec), then
 *      every bar gets a single chord decision from Essentia,
 ...
 * `bpm` and `timeSignature` determine bar length — Essentia gets one chord
 * decision per bar, exactly like the structure editor's "Detect" button
 * (quantizeChordsByBar in chordsWorker.js).
 */
export function analyzeChordsAndKey(
  ...
  const barCount = Math.max(1, Math.ceil(durationSeconds / barDurSec))
  const barChords: string[] = []
  for (let bar = 0; bar < barCount; bar++) {
    const barStartSample = bar * barDurSec * sampleRate
    const barEndSample = Math.min((bar + 1) * barDurSec * sampleRate, pcm.length)
  ...
  for (let bar = 0; bar < barChords.length; bar++) {
    ...
    chords.push({
      timestamp_ms: Math.round(bar * barDurSec * 1000),
      chord,
    })
    prev = chord
  }
...
/** bar = floor(timestamp_seconds / barDurationSec) + 1 */
export function barNumberForTimestamp(
  timestampMs: number,
  bpm: number,
  timeSignature: string = DEFAULT_TIME_SIGNATURE,
): number {
  const barDurSec = barDurationSec(bpm, timeSignature)
  return Math.floor(timestampMs / 1000 / barDurSec) + 1
}
